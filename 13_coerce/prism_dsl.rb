# Stage 13 — 正規化:由 host 決定【統一 string 與 symbol】(coerce)
#
# 前面 type_check(:set, key: Symbol) 是嚴格的:寫 set "base", 1 會被擋,因為
# "base" 是 String 不是 Symbol。但很多 config DSL 想讓 :base 和 "base" 是同義詞
# (key 用哪種寫法都行)。問題是:該不該統一、往哪個方向折、哪些參數要折 ——
# 這都是【政策】,不該由引擎硬寫死。於是延續 Stage 12 的骨架:把決定權交給
# host,引擎自己【一個值都不正規化】,只跑 host 寫的 coercer。
#
# 接口:coerce(name, param: coercer)(跟 dsl_method / type_check / operator
# 並列的第四套)。coercer 是個收單一值的可呼叫物(host 寫的 lambda):
#   coerce(:set, key: ->(v) { v.is_a?(String) ? v.to_sym : v })   # String→Symbol
# host 決定三層:哪些方法/參數要正規化、往哪折(symbol?還是 string?)、
# 哪裡【不】折(沒宣告 coerce 的參數維持嚴格,例如 listen 的 host 仍要 String)。
#
# 關鍵是【管線順序】:求值後,先 coerce(正規化)再 type_check(驗證)——
#   eval_value → coerce → type_check → invoke
# 所以 set "base", 1 的流程是:"base"(求值)→ :base(host 折)→ 過 key: Symbol。
# 順序反了就會在折之前先被型別擋掉。這也維持「一個關注點 = 一個接口」:
#   dsl_method=暴露面、coerce=正規化、type_check=驗證、operator=怎麼算。
#
# 安全性自動成立:coercer 跑在【求值之後】,只拿得到已經 inert 的值;危險的 key
# (set File.read(...), 1)在 eval_value 階段就被擋,根本到不了 coercer ——
# 正規化不可能變成後門。host 的 coercer 若 raise(例如對奇怪型別 to_sym 失敗),
# 在這裡被接住記成錯誤,跟 operator body 一致。
#
# 其餘(parse 不 eval、白名單、type_check、operator、插值、巢狀 subspace、
# 錯誤彙整)同前。本階段只新增 coerce 這一層;一元 -x、range 仍之後再說。
...
module PrismDSL
  ...
  class Interpreter
    ...
    # 跑 host 寫的單個 coercer(self = space,跟 dsl_method / operator 一致)。
