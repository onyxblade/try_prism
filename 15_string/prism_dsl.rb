# Stage 15 — 載入【不可信字串】(回到創始目標)+ host↔DSL 純用方法溝通
#
# Stage 7 起,這條設計線(「DSL 即 class」)只認 inline block(靠 block.source_location
# 讀原始檔)。但本專案的創始目標是安全載入【不可信的字串】(設定檔 / 使用者提交的
# 文本)—— 不可信輸入根本不是一個你能 call 的 Ruby block。這一階段補上那條入口:
#   AppConfig.parse_string(source, **ctx)
# 直接把整段程式的【頂層語句】當 DSL body,丟進【同一套】interpreter(同樣的白名單、
# type_check、coerce、stub、錯誤彙整 —— 零新增攻擊面)。它其實比 inline block 那套
# 更簡單也更安全:不碰檔案系統、不依賴 source_location、在 eval/IRB/網路輸入都能用。
#
# 與此呼應的設計取向:host↔DSL【純用方法溝通】,而不是把外部值「注入」成 DSL 看得到
# 的綁定。其實 DSL 這一側早就如此(env/ref 就是「方法式拉值」;Stage 8 的 block
# parameter 注入在 Stage 9 已廢)。對【字串】場景這特別重要:沒有 Ruby 呼叫點去擺
# 參數,但方法不需要 —— host 先註冊好方法,任何字串直接載入,字串靠【調方法】跟 host
# 要東西。per-load 的可信 context(如 secrets)在 Ruby 側 push 進 initialize,再由一個
# dsl_method 暴露(例如 secret(:k));不可信字串永遠【看不到綁定】,只能調方法。
#
# 兩條入口共用 interpreter:parse(inline block)給可信的嵌入式 DSL,parse_string
# 給不可信輸入。其餘(stub 分派、插值、括號、巢狀 subspace…)同前。
#
#   分工:dsl_method=暴露面(無 receiver 的指令/值方法,也是 host↔DSL 的溝通口)、
#         stub=帶 receiver 的方法(按型別)、coerce=正規化、type_check=驗證。
...
module PrismDSL
  ...
  class Space
    class << self
      ...
      # 入口一:解析(非執行)inline block 的原始碼,回傳建好的 Space 實例。
      # 給可信的、嵌在 .rb 裡的 DSL 用(靠 block.source_location 讀原始檔)。
      def parse(*args, **kwargs, &block)
        Interpreter.new(self).run(args, kwargs, block)
      end

      # 入口二:解析(非執行)一段【字串】原始碼 —— 本專案的創始目標,給【不可信
      # 輸入】用(設定檔 / 使用者提交的文本)。整段程式的頂層語句就是 DSL body;
      # *args/**kwargs 是 per-load 的可信 context,送進 initialize(DSL 看不到綁定,
      # 只能透過 host 宣告的 dsl_method 來讀)。走的是【同一套】interpreter。
      def parse_string(source, *args, **kwargs)
        Interpreter.new(self).run_source(source, args, kwargs)
      end
    end
  ...
  class Interpreter
    ...
    # parse_string 的入口:source 是一段字串原始碼。沒有 block、不碰檔案系統 ——
    # 整段程式的頂層語句(ProgramNode#statements)就是 DSL body,走同一套 interpreter。
    def run_source(source, args, kwargs)
      raise ArgumentError, "parse_string 需要一段字串" unless source.is_a?(String)
      @filename = "(字串)"                     # 錯誤訊息用的標籤(無實體檔)
      result = parse_source(source)             # Prism.parse,語法錯誤一次 raise
      space  = build_space_from(@space_class, args, kwargs, result.value.statements, "頂層 parse_string")
      raise_if_errors!
      space
    end
    ...
    def build_space(space_class, args, kwargs, block_node, where)
      build_space_from(space_class, args, kwargs, body_of(block_node, where), where)
    end

    # 真正建 Space 並走訪語句的核心(block 與 string 兩條入口共用)。
    # statements 是一個 StatementsNode(來自 block body 或 ProgramNode)。
    def build_space_from(space_class, args, kwargs, statements, where)
      space = space_class.new(*args, **kwargs)
      frame = Frame.new(self, space, nil)
      space.instance_variable_set(:@__prism, frame)
      begin
        eval_statements(statements, frame)
      ensure
